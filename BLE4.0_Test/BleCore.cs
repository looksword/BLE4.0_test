...
namespace BLE4._0_Test
{
    public class BleCore
    {
        ...
        /// <param name="sender"></param>
        /// <param name="args"></param>
        private void DeviceWatcher_Stopped(Windows.Devices.Bluetooth.Advertisement.BluetoothLEAdvertisementWatcher sender, Windows.Devices.Bluetooth.Advertisement.BluetoothLEAdvertisementWatcherStoppedEventArgs args)
        {
            string msg = "自动发现设备停止";
            this.MessAgeChanged(MsgType.NotifyTxt, msg);
        }
        ...
        public async void FindService()
        {
            ...
            this.CurrentDevice.GetGattServicesAsync().Completed = async (asyncInfo, asyncStatus) =>
            {
                if (asyncStatus == AsyncStatus.Completed)
                {
                    ...
                    this.MessAgeChanged(MsgType.NotifyTxt, "获取服务收集完毕");
        ...
        public async void FindCharacteristic(GattDeviceService gattDeviceService)
        {
            ...
            this.CurrentService.GetCharacteristicsAsync().Completed = async (asyncInfo, asyncStatus) =>
            {
                if (asyncStatus == AsyncStatus.Completed)
                {
                    ...
                    this.MessAgeChanged(MsgType.NotifyTxt, "GattCharacteristic size=" + characteristics.Count);
                    foreach (GattCharacteristic cts in characteristics)
                    {
                        this.CharacteristicAdded(cts);
                    }
    ...
    public enum MsgType { NotifyTxt, BleData, BleDevice }
}
